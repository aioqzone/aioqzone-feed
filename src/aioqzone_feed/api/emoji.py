...
async def sub(
    ...
    if tasks:
        ...
        if excs: raise RuntimeError(*excs)
    return ''.join(r)
...
async def trans_detail(feed: FeedContent) -> FeedContent:
    tasks = [(t := asyncio.create_task(trans_tag(getattr(feed, i))))
             and t.add_done_callback(lambda t: setattr(feed, i, t.result())) or t
...
async def trans_html(txtbox: Union[HtmlElement, str]) -> HtmlElement:
    if isinstance(txtbox, str): txtbox = fromstring(txtbox)
    imgs: list[HtmlElement] = txtbox.cssselect('img')
    tasks = [
        (t := asyncio.create_task(qe.query(int(m.group(1)), default=build_tag))) and \
        t.add_done_callback(
            lambda t, e=i: \
                setattr(e, 'tail', t.result() + (e.tail or '')) or \
                e.drop_tree()
        ) or t
        for i in imgs if (m := URL_RE.match(i.get('src', '')))
    ]
    if tasks:
        done, pending = await asyncio.wait(tasks)
        assert not pending
